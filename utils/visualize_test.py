...
if __name__ == '__main__':
    # --tracker_name SRZLT_HSE_IPPM_ClipSegmentAnything --workspace /home/vot_workspace
    ...
    for i in range(len(all_folders_names)):
        ...
        for bin_file in bin_file_paths:
            ...
            if len(bin_file_paths) == 1:
                object_number = 1
                gt_file = workspace_path + "/sequences/{}/groundtruth.txt".format(folder_name)
            else:
                object_number = int(os.path.basename(bin_file).split("obj")[1].split('_')[0])
                gt_file = workspace_path + "/sequences/{}/groundtruth_obj{}.txt".format(folder_name, object_number)
